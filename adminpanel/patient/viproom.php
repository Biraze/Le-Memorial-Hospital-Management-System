<?php
        ...
        {	
            ...
            elseif(!empty($r4))
                    {echo '<script>alert("Already Exist in vip")</script>';}
                        ...
                        //
                            $ck=mysqli_query($conn,"select roomno from vroom where roomno='$r'");
                            ...
                            {
                                $rm="insert into vroom(id,roomno)values('$i','$r')";
                                if(mysqli_query($conn,$rm))
                                {
                                    $sql="insert into vip(id,date)values('$i','$date')";
                                    ...
                                    {
                                        $s="select id from vip where id='$i'";
    ...
    if(isset($_POST['delete']))
    {
            ...
            $dep='vip';
            $test = mysqli_query($conn,"select id from patientreg where id='$i'");
            $test1 = mysqli_query($conn,"select id from vroom where id='$i'");
            $r0 = mysqli_fetch_row($test);
            ...
            {	
                if(!empty($r1))
                {
                $getinfo = "select date from vip where id='$i'";
                $query = mysqli_query($conn,$getinfo);
                    ...
                    if(mysqli_query($conn,$s))
                    {
                        mysqli_query($conn,"delete from vroom where id='$i'");
                        
                        if(mysqli_query($conn,"delete from vip where id='$i'"))
                        {
                            {echo '<script>alert("Successful")</script>';}
                        }
...
<html lang="en">
    ...
    <form action='viproom.php' method='post'>
        <h1>VIP Room Registration Form</h1>
        <br><h2 id="vroom-admit">Admitting</h2>
        <p>
            <label>ID</label>
    ...
    <form action='viproom.php' method='post'>
        <div id="vroom-discharge"><h2 >Discharging</h2></div><br>
        <p>
            <label>ID</label>
            <input type='text' name='id' placeholder="Patientfirstletter,year,formno eg:N20201">
...
<?php
    if(isset($_POST['view']))
    {
        $result = mysqli_query($conn,"select patientreg.id,patientreg.name,vroom.roomno from patientreg join vip on patientreg.id=vip.id join vroom on vip.id=vroom.id");
    ...
    if(isset($_POST['hide']))
    {
        $result = mysqli_query($conn,"SELECT * FROM `totalroom` WHERE roomno1 not in(SELECT roomno FROM vroom) AND section='vipROOM'");
        echo"<table>
