<?php
            ...
            {	
                ...
                $r4 = mysqli_fetch_row($check4);
                    ...
                    elseif(!empty($r4))
                        ...
                        //
                            $ck=mysqli_query($conn,"select roomno from icuroom where roomno='$r'");
                            ...
                            {
                                $rm="insert into icuroom(id,roomno)values('$i','$r')";
                                if(mysqli_query($conn,$rm))
                                {
                                    $sql="insert into icu(id,date)values('$i','$date')";
                                    ...
                                    {
                                        $s="select id from icu where id='$i'";
    ...
    if(isset($_POST['delete']))
    {
        ...
        $dep='ICU';
        $test = mysqli_query($conn,"select id from patientreg where id='$i'");
        $test1 = mysqli_query($conn,"select id from icu where id='$i'");
        $r0 = mysqli_fetch_row($test);
        ...
        {	
            if(!empty($r1))
            {
            $getinfo = "select date from icu where id='$i'";
            $query = mysqli_query($conn,$getinfo);
                ...
                if(mysqli_query($conn,$s))
                {
                    mysqli_query($conn,"delete from icuroom where id='$i'");
                    
                    if(mysqli_query($conn,"delete from icu where id='$i'"))
                    {
                        {echo '<script>alert("Successful")</script>';}
                    }
...
<html lang="en">
    ...
    <body>
        ...
        <form action='icu.php' method='post'>

            <h1>ICU Registration Form</h1>
            <br><h2>Admitting</h2>
            <p>
        ...
        <form action='icu.php' method='post'>
            ...
            <br><h2 id="discharging">Discharging</h2>
            <p>
            <label>ID</label>
            <input type='text' name='id' placeholder="Patientfirstletter,year,formno eg:N20201">
...
<?php
    if(isset($_POST['view']))
    {
        $result = mysqli_query($conn,"select patientreg.id,patientreg.name,icuroom.roomno from patientreg join icu on patientreg.id=icu.id join icuroom on icu.id=icuroom.id");
    ...
    if(isset($_POST['hide']))
    {
        $result = mysqli_query($conn,"SELECT * FROM `totalroom` WHERE roomno1 not in(SELECT roomno FROM icuroom) AND section='ICU BED'");
